...
const UUID = process.env.UUID || '6d0c8da2-3189-47c4-bbbc-83a38f35212d';            
const AUTO_ACCESS = process.env.AUTO_ACCESS || false;      
const SUB_PATH = process.env.SUB_PATH || 'sub';            
const XPATH = process.env.XPATH || 'xxxx';                
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/') {
        res.writeHead(302, { 'Location': 'https://wwww.hidns.co' });
        res.end();
        return;
    } 
    
    if (req.url === `/${SUB_PATH}`) {
        const vlessURL = `vless://${UUID}@${IP}:443?encryption=none&security=tls&sni=${IP}&fp=chrome&allowInsecure=1&type=xhttp&host=${IP}&path=%2F${XPATH}&mode=packet-up#${NAME}-${ISP}`; 
